...
RUN mkdir /home/CM
RUN git clone https://github.com/schubmat/Custom-MADE.git /tmp/Custom-MADE
RUN git clone https://github.com/schubmat/Custom-MADE-DSLs.git /tmp/Custom-DSLs
RUN echo "cp -r /tmp/Custom-DSLs" > /tmp/Custom-MADE/backend/language-repository.git.cnf
WORKDIR "/tmp/"
# RUN bash -x run.sh
