...
public class VersionController {
    ...
    public ResponseEntity all(@AuthenticationPrincipal User user) {
        return ResponseEntity.ok(versionRepository.findAll()
                .stream()
                .filter(version -> version.getMemberships()
                        .stream()
    ...
    public ResponseEntity create(@AuthenticationPrincipal UserDetails userDetails,
        ...
        if (!optionalGrammar.isPresent()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The selected grammar does not exist");
        }
    ...
    public ResponseEntity putMembersSettings(
            @AuthenticationPrincipal UserDetails userDetails,
            @PathVariable long versionId,
    ...
    public ResponseEntity export(@AuthenticationPrincipal UserDetails userDetails,
                                 @PathVariable long id,
                                 @Valid @RequestBody File[] requestedFiles) {
        User user = userRepository.findByUsername(userDetails.getUsername()).get();
        if (!validate(user, id, Permissions.EXPORT_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exists or user does not have permission to export files");
        Version version = versionRepository.findById(id).get();
        if (!version.getGrammar().isHasGenerator()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The grammar does not support any generation");
        }
        List<File> files = Arrays.stream(requestedFiles)
                .map(file -> fileRepository.findById(file.getId()).get())
    ...
    @PostMapping("/export")
    public ResponseEntity exportVersions(@AuthenticationPrincipal User user, @Valid @RequestBody Version[] requestVersions) {
        List<Version> versions = Arrays.stream(requestVersions)
                .map(version -> versionRepository.findById(version.getId()).get())
    ...
    public ResponseEntity add(@AuthenticationPrincipal UserDetails userDetails,
                              @PathVariable long id,
                              @Valid @RequestBody final File request) {
        User user = userRepository.findByUsername(userDetails.getUsername()).get();
        if (!validate(user, id, Permissions.CHANGE_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exists or user does not have permission to add files");
        ...
        if (version.getFiles().stream().noneMatch(m0File -> m0File.getName().equals(request.getName()))) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("File with this name already exists in project");
        }
    }
    ...
    public ResponseEntity download(@AuthenticationPrincipal UserDetails userDetails,
        ...
        User user = userRepository.findByUsername(userDetails.getUsername()).get();
        if (!validate(user, id, Permissions.BROWSE_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exists or user does not have permission to browse files");
        Version version = versionRepository.getOne(id);
    ...
    public ResponseEntity delete(
        ...
        if (!validate(user, id, Permissions.DELETE_VERSION)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exist or user does not have permission to delete it");
        }
        Version version = versionRepository.getOne(id);
    ...
    public ResponseEntity uploadFiles(@AuthenticationPrincipal UserDetails userDetails,
                                      @PathVariable long id,
        ...
        if (modifiedFiles.isEmpty())
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Request did not contain any appropriated file: file extension has to match \""+version.getGrammar().getDslExtension()+"\".");
        ...
        try {
            gitService.commitPullAndPushChanges(modifiedFiles.toArray(new File[modifiedFiles.size()]), "added/edited " + String.join(", ", fileNames), user, version);
    ...
    public ResponseEntity setFiles(@AuthenticationPrincipal UserDetails userDetails,
                                   @PathVariable long versionId,
                                   @RequestBody File[] files)  {
        User user = userRepository.findByUsername(userDetails.getUsername()).get();
        if (!validate(user, versionId, Permissions.CHANGE_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exist or user does not have permission to edit files");
        Version version = versionRepository.getOne(versionId);
        ...
        try {
            ...
            try {
                ...
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Unable to restore file backup. The project is probably broken now, please contact the allmighty admin!");
            }
        } finally {
            try {
                fileSystem.removeBackup();
        ...
        String[] fileNames = modifiedFiles.stream().map(file -> file.getName()).toArray(String[]::new);
        try {
            gitService.commitPullAndPushChanges(modifiedFiles.toArray(new File[modifiedFiles.size()]), "edited " + String.join(", ", fileNames), user, version);
    ...
    public ResponseEntity deleteFiles(@AuthenticationPrincipal User user,
                                      @PathVariable long versionId,
                                      @RequestBody File[] requestedFiles)  {
        if (!validate(user, versionId, Permissions.CHANGE_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exist or user does not have permission to delete files");
        Version version = versionRepository.getOne(versionId);
        List<File> files = Arrays.stream(requestedFiles)
                .map(file -> fileRepository.findById(file.getId()).get())
                .filter(Objects::nonNull)
                .filter(file -> file.getVersion().getId() == versionId).collect(Collectors.toList());
        for (File file : files) {
            file.setStatus(FileStatus.UNCHECKED);
        ...
        try {
            ...
            gitService.commitPullAndPushChanges(files.toArray(new File[files.size()]),"deleted "+String.join(", ", fileNames), user, version);
        } catch (GitException | IOException e) { }

        List<File> conflictFiles = files.stream().filter(file -> file.getStatus() == FileStatus.IN_CONFLICT).collect(Collectors.toList());
        oneSidedDeleteConflict(conflictFiles, false);
        return ResponseEntity.ok(new VersionDTOBuilder(version).build());
    }

    @PatchMapping("{id}/validate")
    public ResponseEntity validate(@AuthenticationPrincipal UserDetails userDetails,
                                   @PathVariable long id) {
        User user = userRepository.findByUsername(userDetails.getUsername()).get();
        if (!validate(user, id, Permissions.BROWSE_FILES))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exists or user does not have permission to browse files");
    ...
    public ResponseEntity validate(@AuthenticationPrincipal UserDetails userDetails,
        ...
        List<File> files = Arrays.stream(requestedFiles)
                .map(file -> fileRepository.findById(file.getId()).get())
                .filter(Objects::nonNull)
                .filter(file -> file.getStatus() != FileStatus.IN_CONFLICT).collect(Collectors.toList());
    ...
    @PatchMapping("{id}/git")
    public ResponseEntity setGitConfig(@AuthenticationPrincipal User user,
                                       @PathVariable long id,
                                       @Valid @RequestBody GitConfiguration gitConfig) {
        if (!validate(user, id, Permissions.CONFIG_GIT))
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Version does not exist or user does not have permission to change the git config");
        if (!gitConfig.isValid())
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Unvalid configuration (check key, url and branch)");
        Version version = versionRepository.getOne(id);
        ...
        try {
            ...
            try {
                gitService.commitPullAndPushChanges(files.toArray(new File[files.size()]),"initialized with Custom-MADE", user, version);
            } catch (IOException e1) {
